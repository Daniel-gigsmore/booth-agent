...
**Access model:** booth-agent runs locally on hardware you control, not in a browser, so it connects with the Supabase **service_role** key (`supabase.serviceRoleKey` in `booth.config.json`), not anon - service_role bypasses RLS by default, so it can insert/update `captures` and read/write the `captures` bucket with no explicit grants needed. Only a guest-facing web app (not part of this repo) should ever hold an anon key; the migration grants that role **read-only** access - `SELECT` on `captures` and a read-only storage policy scoped to the bucket. If a guest app later needs to write anything (e.g. a "favorite" or share action), extend the policies deliberately rather than widening these.
...
| `compositing.templateDir` | Where `<templateId>.json` template files and their overlay PNGs live. 4x6 cells are 1200x1800 (portrait) or 1800x1200 (landscape, turned onto the sheet at print time); 2x6-strip cells are 600x1800. Each photo slot takes one shot, so the slot count is how many photos a guest takes. Copy `assets/templates/*.json` here on setup: `default-4r-grid` (landscape, 4 photos), `default-4r-three` (landscape, 1 big + 2 small), `default` (portrait, 1 photo), `default-strip` (2x6 strips, 3 photos). The kiosk's layout editor adds more. |
...
- `GET /captures/:id/image` - the original JPEG for a capture, so the kiosk can show it on the review screen (`/capture` only returns a local path a browser cannot open). Add `?variant=composite` for the print-ready composite instead. Use `?token=` from an `<img src>`. `404` if the id is unknown, it has not been composited yet (for `variant=composite`), or the file is gone.
...
- `GET /templates` - every layout in `compositing.templateDir`. `POST /templates/:id` saves one (body is the template JSON; validated for cell size and slots that run off the edge). `POST /templates/:id/delete` removes one (`409` if it's the layout in use). `POST /templates/:id/overlay` uploads a PNG overlay (`Content-Type: image/png`, max 10 MB); `GET /templates/:id/overlay` serves it. These back the kiosk's layout editor. Everything is POST so the CORS allowlist stays GET/POST.
- `GET /session` / `POST /session` - `{ templateId, firstCountdownSeconds, betweenShotsSeconds }` (seconds 1-10), set from the kiosk's operator panel. Stored in `<dataDir>\session.json`, not `booth.config.json`. `GET` also returns the resolved `template`; its number of photo slots is how many shots the kiosk takes. Defaults: `default-4r-grid`, 3 s, 3 s.
